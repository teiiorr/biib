...
for (const theme of THEMES) {
  test(`panel burchaklari ${theme}`, async ({ page }, testInfo) => {
    ...
    for (const [t, d] of CORNERS) {
      ...
      const file = evidenceFile(testInfo, "G6", theme, `panel-t${t}-d${d}.png`);
      await page.screenshot({ path: file });
      await page.keyboard.press("Escape");
    }
    await recordCheck(testInfo, "G6", {
      id: checkId(testInfo, "panel-corners", theme),
      status: failures.length ? "fail" : "pass",
      detail: summarize(failures),
    });
    expect(failures).toEqual([]);
...
test("klaviatura yoʻli", async ({ page, isMobile }, testInfo) => {
  ...
  await page.goto(pathFor("uz", "home"));
  await settle(page);
  const reducedShot = evidenceFile(testInfo, "G6", "reduced-motion.png");
  await page.screenshot({ path: reducedShot, fullPage: false });
  await page.emulateMedia({ reducedMotion: null });

  await recordCheck(testInfo, "G6", {
    id: checkId(testInfo, "keyboard"),
    status: failures.length ? "fail" : "pass",
    detail: summarize(failures),

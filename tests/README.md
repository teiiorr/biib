...
Muhit: `BASE_URL` (sukut http://localhost:3100), `DESIGN=atlas|birlashma|both`.

## Sahifa quruvchilar uchun belgilar

`data-testid`: `skip-link`, `header`, `tab-bar`, `menu-sheet`, `appearance-open`, `appearance-panel`,
`slider-transparency`, `slider-density`, `design-atlas`, `design-birlashma`, `language-open`,
`language-menu`, `contact-form`, `footer-crown`, `portal-scene`.
